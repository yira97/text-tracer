...
namespace bys {
    ...
    private:
        ...
        std::array<double,maxStatusesType> ppt{0};
    public:
        Node() = default;
        Node(std::string name) : name(std::move(name)) {}
        ~Node() = default;
        
        void setP(NODE_STATUS type, double value){
            if (statuses[type] == true){
                ppt[type] = value;
            } else {
                log("statuses[type] == false","fail to set probability");
            }
        }
    ...
    //  introduce terminology node for compute the  weight of Priori probability
